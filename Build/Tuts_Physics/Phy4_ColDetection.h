...
public:
	...
	bool m_cube_collidig = false;
	bool m_sphere_sphere_colliding = false;
	bool m_sphere_cube_colliding = false;
	...
	virtual void OnInitializeScene() override
	{
		...
		//Create Sphere-Sphere Manifold Test
		{
			...
			sphere->Physics()->SetOnCollisionCallback([&](PhysicsObject* self_obj, PhysicsObject* other_obj) {
				m_sphere_sphere_colliding = true;
				return true;
			});
	...
	virtual void OnUpdateScene(float dt) override
	{
		...
		if (m_Rotating)
		{
			...
			if (orbiting_sphere1 != NULL)
			{
				...
				if (m_sphere_sphere_colliding) {
					orbiting_sphere1->SetColour(Vector4(1.f, 0.f, 1.f, 0.5f));
				}
				else {
					orbiting_sphere1->SetColour(Vector4(0.5f, 0.f, 0.f, 0.5f));
				}
				m_sphere_sphere_colliding = false;
			}
			...
			if (orbiting_sphere2 != NULL)
			{
				...
				if (m_sphere_cube_colliding) {
					orbiting_sphere2->SetColour(Vector4(1.f, 0.f, 1.f, 1.f));
				}
				else {
					orbiting_sphere2->SetColour(Vector4(0.5f, 0.f, 0.f, 1.f));
				}
				m_sphere_cube_colliding = false;
			}
			...
			if (rotating_cuboid1 != NULL)
			{
				...
				if (m_cube_collidig) {
					rotating_cuboid1->SetColour(Vector4(1.f, 0.f, 1.f, 0.5f));
				}
				else {
					rotating_cuboid1->SetColour(Vector4(0.5f, 0.f, 0.f, 0.5f));
				}
				m_cube_collidig = false;
			}
		}

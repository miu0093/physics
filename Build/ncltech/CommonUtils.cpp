	...
	const Vector4& color)
{
	...
	pCuboid->SetTexture(CommonMeshes::ScoreBoardTex(), false);
...
Object* CommonUtils::BuildCuboidGroundFloor(
	const std::string& name,
	const Vector3& pos,
	const Vector3& halfdims,
	bool physics_enabled,
	...
	const Vector4& color)
{
	...
	pCuboid->SetMesh(CommonMeshes::Cube(), false);
	pCuboid->SetTexture(CommonMeshes::GroundFloor(), false);
	pCuboid->SetLocalTransform(Matrix4::Scale(halfdims));
	pCuboid->SetColour(color);
	pCuboid->SetBoundingRadius(halfdims.Length());
...
//Object* CommonUtils::BuildSphereObjectIO(
//	const std::string& name,
//	const Vector3& pos,
//	float radius,
//	bool physics_enabled,
...
//	const Vector4& color)
//{
//	ObjectMesh* pSphereIO = dragable
//		? new ObjectMeshDragable(name)
//		: new ObjectMesh(name);
//
//	pSphereIO->SetMesh(CommonMeshes::Sphere(), false);
//	pSphereIO->SetTexture(CommonMeshes::IO(), false);
//	pSphereIO->SetLocalTransform(Matrix4::Scale(Vector3(radius, radius, radius)));
//	pSphereIO->SetColour(color);
//	pSphereIO->SetBoundingRadius(radius);
//
//	if (!physics_enabled)
//	{
//		//If no physics object is present, just set the local transform (modelMatrix) directly
//		pSphereIO->SetLocalTransform(Matrix4::Translation(pos) * pSphereIO->GetLocalTransform());
//	}
//	else
//	{
//		//Otherwise create a physics object, and set it's position etc
//		pSphereIO->CreatePhysicsNode();
//
//		pSphereIO->Physics()->SetPosition(pos);
//		pSphereIO->Physics()->SetInverseMass(inverse_mass);
//
//		if (!collidable)
//		{
//			//Even without a collision shape, the inertia matrix for rotation has to be derived from the objects shape
//			pSphereIO->Physics()->SetInverseInertia(SphereCollisionShape(radius).BuildInverseInertia(inverse_mass));
//		}
//		else
//		{
//			CollisionShape* pColshape = new SphereCollisionShape(radius);
//			pSphereIO->Physics()->SetCollisionShape(pColshape);
//			pSphereIO->Physics()->SetInverseInertia(pColshape->BuildInverseInertia(inverse_mass));
//		}
//	}
//
//	return pSphereIO;

...
{/*//semi-implicit 
	...
	Vector3 g = obj->GetPosition() - Vector3(1.f, 20.0f, 0.0f);//centre of the ball
	g.Normalise();
	m_Gravity = g * (-9.81f);
	...
	if (obj->m_Rested == true) {
		obj->m_AngularVelocity = Vector3(0.f, 0.f, 0.f);
		obj->m_LinearVelocity  = Vector3(0.f, 0.f, 0.f);
	}

	Vector3 m_tempVelocity;
	m_tempVelocity = obj->m_LinearVelocity;
	//1. velocity in next time step
	obj->m_LinearVelocity += obj->m_Force * obj->m_InvMass * m_UpdateTimestep;
	obj->m_LinearVelocity = obj->m_LinearVelocity * m_DampingFactor;
	//2. position in next time step
	//obj->m_Position += obj->m_LinearVelocity * m_UpdateTimestep;
	obj->m_Position += (obj->m_LinearVelocity + m_tempVelocity) * m_UpdateTimestep *0.5;

	//Midpoint Method (angular displacement)
	m_tempVelocity = obj->m_AngularVelocity;
	//1. velocity
	obj->m_AngularVelocity += obj->m_InvInertia * obj->m_Torque * m_UpdateTimestep;
	obj->m_AngularVelocity = obj->m_AngularVelocity * m_DampingFactor;
	//2. position
	...
	if (m_BroadphaseCollisionPairs.size() > 0)
	{
		...
		for (size_t i = 0; i < m_BroadphaseCollisionPairs.size(); ++i)
		{
			...
			if (colDetect.AreColliding(&colData))
			{
				...
				if (okA && okB)
				{
					Vector3 m_posObjA = cp.pObjectA->GetPosition();
					Vector3 m_posObjB = cp.pObjectB->GetPosition();

					if (cp.pObjectA->m_bullet && cp.pObjectB->m_target && cp.pObjectA->m_getpoint == false) {
						Vector3 deltaV = m_posObjA - m_posObjB;
						float distance = deltaV.LengthSquared();

						m_boardscore = 10.f*(1.f / distance);
						cp.pObjectA->m_getpoint = true;
					}
					else if (cp.pObjectA->m_target && cp.pObjectB->m_bullet && cp.pObjectB->m_getpoint == false) {
						Vector3 deltaV = m_posObjA - m_posObjB;
						float distance = deltaV.LengthSquared();

						m_boardscore = 10.f*(1.f / distance);
						cp.pObjectB->m_getpoint = true;
					}

					cp.pObjectA->m_Collisioned = true;
					cp.pObjectB->m_Collisioned = true;

					if (!(cp.pObjectA->RestState()) || !(cp.pObjectB->RestState())) {
						cp.pObjectA->m_Rested = false;
						cp.pObjectB->m_Rested = false;
					}
					//-- TUTORIAL 5 CODE --
